...
from Game2.engine.updater import render_frame, only_render

screensize = (screensize[0], screensize[1])
unt = ["gunner ", "arty ", "fighter ", "starbase "]
...
def main():
    ...
    while running:
        ...
        render_frame(entitylist, screen) if not pause else only_render(entitylist, screen)
        for event in pygame.event.get():
            ...
            if state[0] != 0:
                ...
                if chosen_unit == 3:
                    entitylist.append(Starbase(*pygame.mouse.get_pos(), starbase1, hp=1500,
            ...
            if state[2] != 0:
                ...
                if chosen_unit == 2:
                    entitylist.append(Unit(*pygame.mouse.get_pos(), fighterimg_2, hp=5,
                                           ...
                                           projectile_death=None, spd=2, firerate=60 * (.125), unit_spd=8))
                if chosen_unit == 3:
                    entitylist.append(Starbase(*pygame.mouse.get_pos(), starbase2, hp=1500,
        ...
        if cntr % 60 == 0:
            fps = sum(fps)/len(fps)
            txt_surface = font.render(str(round(1/fps)) + "fps", True, (0, 0, 255))
